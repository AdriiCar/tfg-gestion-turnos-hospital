...
interface RotacionProps {
    ...
    añadirUsuario: (grupoId: string, correo: string) => Promise<{exito: boolean; mensaje: string}>;
...
//AÑADMOS A UN USUARIO A UN GRUPO
    const confirmarAñadirUsuario = async () => {
        ...
        empezarTransicion(async()=> {
            try{
                const resultado = await añadirUsuario(grupoSeleccionado, datosUsuario.correo);
                if(resultado.exito){
    ...
    return (
        <Box p= "6">
            ...
            <Flex justify="center" mb="6">
                <Button size="4" disabled={estaPendiente} onClick={abrirResumenGeneracion} style={{ backgroundColor: "#0284C7", cursor: "pointer", padding: "0 40px" }}>
            ...
            <Dialog.Root open={dialogoGenerar} onOpenChange={setDialogoGenerar}>
                <Dialog.Content style= {{maxWidth: 400}}>
                    ...
                    <Flex justify="end" gap="3" mt="5">
                        ...
                        <Button style={{ backgroundColor: "#0284C7", cursor: "pointer", minWidth: "120px" }} disabled={estaPendiente} onClick={() => setDialogoGenerar(false)}>
                            <PlayIcon/> Ejecutar
                        </Button>
                    </Flex>
                </Dialog.Content>
                    ...
                    <Dialog.Title size="5" mb="4">Asignar Empleado a Grupo</Dialog.Title>
                     <Flex direction="column" gap="4">
                        ...
                        <Flex justify="end" gap="3" mt="4">
                            <Dialog.Close><Button variant="soft" color="gray" disabled={estaPendiente} style={{cursor: "pointer"}}>Cancelar</Button></Dialog.Close>
                            <Button onClick={confirmarAñadirUsuario} disabled={estaPendiente} style={{ cursor: "pointer", backgroundColor: "#0088CC" }}>Asignar</Button>
                        </Flex>
                    </Flex>
                </Dialog.Content>
            </Dialog.Root>
